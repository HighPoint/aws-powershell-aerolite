...
First, Aerolite opens a text file in an S3 bucket. It removes the comments, creating a list of commands. Next, Aerolite executes each PowerShell command individually. It waits for confirmation that the command successfully executed or failed. If the command failed, it will retry up to three times. Aerolite will log all the errors to CloudWatch. If the command requires an EC2 restart, Aerolite will wait for the EC2 to be available for PowerShell commands.
